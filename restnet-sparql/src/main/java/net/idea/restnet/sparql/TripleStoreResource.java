...
public class TripleStoreResource  extends AbstractResource {
	...
	@Override
	public StringConvertor createConvertor(Variant variant) throws AmbitException,
			ResourceException {

		model = createOntologyModel(true);
		if (MediaType.TEXT_HTML.equals(variant.getMediaType())) {
			return new StringConvertor(new SPARQL2HTMLReporter(model,variant.getMediaType(),getRequest(),getHTMLBeauty()), variant.getMediaType(), null);
		} else {
			return new StringConvertor(new StreamSPARQLReporter(model,variant.getMediaType(),getRequest()), variant.getMediaType(), null);
		}
	}
	...
	@Override
	protected String createQuery(Context context, Request request, Response response)
			throws ResourceException {
		Form form = getRequest().getResourceRef().getQueryAsForm();
		Object query = form.getFirstValue("query");
		String uri = form.getFirstValue("uri");
		return 	query==null?
				uri==null?"SELECT ?s ?p ?o where { ?s ?p ?o } \n limit 1000":
				String.format("SELECT ?s ?p ?o where { <%s> ?p ?o. } \n",uri):
				query.toString();
	}
		...
		for (String owl:owls)
			try {
				readOWL(getClass().getClassLoader().getResourceAsStream(
						String.format("ambit2/rest/owl/%s",owl)),
				ontology);
			} catch (Exception x) {
