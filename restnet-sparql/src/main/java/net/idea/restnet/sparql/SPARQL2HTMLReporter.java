...
public class SPARQL2HTMLReporter extends AbstractSPARQLReporter<Writer> {
	...
	public void header(Writer writer, String query) {
		try {
			...
			writer.write(String.format("<a href=\"javascript:toggleDiv('%s');\" style=\"background-color: #fff; padding: 5px 10px;\">New resource</a>\n","input"));
			writer.write(String.format("&nbsp;<a href=\"javascript:toggleDiv('%s');\" style=\"background-color: #fff; padding: 5px 10px;\">SPARQL</a>\n","sparql"));
	...
	@Override
	public void processResults(Query query, ResultSet results, Writer output)
			throws Exception {
	...
	@Override
	public void processItem(QuerySolution solution, List<String> vars, Writer writer)
			throws Exception {
		writer.write("<tr>");
