...
public class UserJSONReporter<Q extends IQueryRetrieval<DBUser>> extends QueryReporter<DBUser, Q, Writer> {
	...
	public UserJSONReporter(Request request, String usersdbname) {
		...
		if (queryR != null) {
			MasterDetailsProcessor<DBUser, String, IQueryCondition> roleReader = new MasterDetailsProcessor<DBUser, String, IQueryCondition>(
					queryR) {
				public DBUser process(DBUser target) throws Exception {
				...
				protected DBUser processDetail(DBUser target, String role) throws Exception {
					if (target.getID() > 0) {
						if (target.getRoles() == null)
							target.setRoles(new ArrayList<String>());
		...
		if (usersdbname != null) {
			...
			MasterDetailsProcessor<DBUser, UserRegistration, IQueryCondition> regReader = new MasterDetailsProcessor<DBUser, UserRegistration, IQueryCondition>(
					queryReg) {
				@Override
	...
	public Object processItem(DBUser user) throws Exception {
		try {
			...
			if (user.getOrganisations() != null)
				for (Organisation org : user.getOrganisations()) {
					if (group == null)
						...
						group.append(",");
					group.append(String.format(formatGroup, groupURIReporter.getURI(org),
			...
			getOutput().write(String.format(format, uri,
					...
					user.getKeywords() == null ? "" : JSONUtils.jsonEscape(user.getKeywords()), user.isReviewer(),
					user.getRegistrationStatus() == null ? null
							: JSONUtils.jsonQuote(JSONUtils.jsonEscape(user.getRegistrationStatus().name())),
					group == null ? "" : group.toString(), roles));
			comma = ",";
		} catch (IOException x) {
	...
	// output.write("Title,First name,Last name,user
	// name,email,Keywords,Reviewer\n");
