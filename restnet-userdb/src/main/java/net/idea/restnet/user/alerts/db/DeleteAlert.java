...
public class DeleteAlert extends AbstractAlertUpdate<DBUser> {
	...
	if (params.size() == 0)
	    throw new AmbitException("Both alert and user are missing!");
	return params;
	...
	if (getObject() != null && getObject().getID() > 0)
	    alert = DBAlert._fields.idquery.getCondition();

	if (getGroup() != null)
	    if (getGroup().getID() > 0)
		user = DBAlert._fields.iduser.getCondition();
	    else if (getGroup().getUserName() != null) {
		user = " username=? ";
		sql = sql_username;
	    }

	return new String[] { String.format(sql, alert == null ? "" : alert, alert != null && user != null ? " and "
		: "", user == null ? "" : user) };
    }
