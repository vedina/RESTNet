package net.idea.restnet.user.alerts.db;
...
public class AddAlert extends AbstractAlertUpdate<DBUser> {
	public static final String[] sql_addAlert_byuserid = new String[] {
			"insert into alert (name,query,qformat,rfrequency,rinterval,iduser,sent) values (?,?,?,?,?,?,now()) " };
	public static final String[] sql_addAlert_byusername = new String[] {
			"insert into alert " + "SELECT null,?,?,?,?,?,iduser,now(),now() from user where username=? " };
	...
	public List<QueryParam> getParameters(int index) throws AmbitException {
		...
		params.add(new QueryParam<String>(String.class,
				getObject().getTitle() == null ? getObject().getQuery().getContent() : getObject().getTitle()));
	...
	public String[] getSQL() throws AmbitException {
		if (getGroup() == null)
			...
			return sql_addAlert_byuserid;
		else if (getGroup().getUserName() != null)
			return sql_addAlert_byusername;
		throw new InvalidUserException();
	}
