...
public class UserRDFReporter<Q extends IQueryRetrieval<DBUser>> extends QueryRDFReporter<DBUser, Q> {
	...
	MasterDetailsProcessor<DBUser, DBProject, IQueryCondition> projectReader = new MasterDetailsProcessor<DBUser, DBProject, IQueryCondition>(
		queryP) {
	    @Override
	    protected DBUser processDetail(DBUser target, DBProject detail) throws Exception {
		detail.setResourceURL(new URL(groupURIReporter.getURI(detail)));
	...
	if (output != null) {
	    ...
	    output.setNsPrefix("tbpt",
		    String.format("%s%s/", uriReporter.getBaseReference().toString(), Resources.project));
	    output.setNsPrefix("tbo",
		    String.format("%s%s/", uriReporter.getBaseReference().toString(), Resources.organisation));
	...
	try {
	    ...
	    ioClass.objectToJena(getJenaModel(), // create a new class
		    item);
	    return item;
	} catch (Exception x) {
