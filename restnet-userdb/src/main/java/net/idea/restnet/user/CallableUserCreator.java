...
public abstract class CallableUserCreator extends CallableDBUpdateTask<DBUser, Form, String> implements IDBConfig {
    ...
    public CallableUserCreator(Method method, DBUser item, UserURIReporter<IQueryRetrieval<DBUser>> reporter,
	    ...
	    boolean enableEmailVerification, String usersdbname) {
	...
	if (passwordChange) {
		...
		credentials = new UserCredentials(input.getFirstValue("pwdold"), input.getFirstValue("pwd1"));
	    ...
	    if (input != null)
		credentials = new UserCredentials(input.getFirstValue("pwd1"), input.getFirstValue("pwd2"));
	}
	if (input == null)
	    return user;
	...
	try {
	    user.setHomepage(new URL(input.getFirstValue(ReadUser.fields.homepage.name())));
	} catch (Exception x) {
	}
	try {
	    user.setWeblog(new URL(input.getFirstValue(ReadUser.fields.weblog.name())));
	} catch (Exception x) {
	}
	...
	if (passwordChange)
		...
		notification.sendNotification(target.getEmail(), String.format("%s (%s %s)", subject,
			target.getFirstname(), target.getLastname()), String.format(emailContent,
			target.getFirstname(), target.getLastname(), getSystemName(), getSystemName(),
			dateFormat.format(registeredAt), baseReference, Resources.register, Resources.confirm,
			registration.getConfirmationCode(), dateFormat.format(cal.getTime()), getSystemName(),
