...
public class CallableUserCreatorTest extends DbUnitTest {
	...
	public void testCreateUser() throws Exception {
		...
		try {
			...
			CallableUserCreator callable = new CallableUserCreator(Method.POST,null,reporter, 
					form,"/",
					c.getConnection(),null,false,
					false,
					"aalocal_test") {
			...
			ITable table = 	c1.createQueryTable("EXPECTED",String.format("SELECT count(*) from user where username='test1'"));
			Assert.assertEquals(1,table.getRowCount());
			table = 	c1.createQueryTable("EXPECTED",String.format("SELECT status from user_registration where user_name='test1'"));
			Assert.assertEquals("confirmed",table.getValue(0,"status"));
