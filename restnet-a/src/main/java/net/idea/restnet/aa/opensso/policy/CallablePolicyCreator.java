...
public class CallablePolicyCreator<USERID> extends CallableProtectedTask<USERID> {
	...
	if (uri == null)
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, "'uri' parameter not found!");
	try {
	    type = _type.valueOf(form.getFirstValue("type"));
	} catch (Exception x) {
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
		    "'type' parameter not found! Should be type=user|group");
	}
	name = form.getFirstValue("name");
	if (name == null)
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
		    "'name' parameter not found! Should be name of the user or group");

	for (_method method : _method.values())
	    try {
	...
	try {
	    ...
	    int j = 0;
	    for (_method x : _method.values())
		if (methods[x.ordinal()]) {
		    m[j] = x.name();
		    j++;
		}
	    ...
	    if (httpcode == 200)
		return new TaskResult(String.format("%s/%s/%s/%s", applicationRootReference, AdminResource.resource,
			OpenSSOPoliciesResource.resource, b.toString()));
	    else
		throw new ResourceException(Status.SERVER_ERROR_BAD_GATEWAY, String.format(
			"The policy service %s returned %d", config.getPolicyService(), httpcode));
