...
public class PolicyResource extends CatalogResource<String> {
	...
	if (uri == null)
	    uri = String.format("%s/%s", getRequest().getRootRef(), UUID.randomUUID());
	if (user == null)
	...
	if (getMethod().equals(Method.GET)) {
		...
		if (authenticate()) {
			...
			try {
			    ...
			    // topics.add(String.format("Authorized=%s METHOD=%s URI=%s",x.getMessage(),
			    // action,uri));
			}
		} else
    ...
    protected Representation createPolicy(Form form, Variant variant, boolean deleteOld) throws ResourceException {
	...
	try {
	    ...
	    // int r = policy.createUserPolicy("nina", ssoToken, uri,
	    // methods,policyid.toString());
		...
		/*
		 * Hashtable<String, String> policies = new Hashtable<String,
		 * String>(); policy.listPolicies(ssoToken, policies);
		 * Enumeration<String> keys = policies.keys(); while
		...
		for (String action : methods)
			...
			if (!ok)
			    throw new ResourceException(Status.CLIENT_ERROR_UNAUTHORIZED, b.toString());
		    } catch (ResourceException x) {
			throw x;
		    } catch (Exception x) {
	    ...
	    if (deleteOld)
		try {
		    policy.deletePolicy(ssoToken, policyid.toString());
		} catch (Exception x) {
		    b.append(x);
		}
	    try {
