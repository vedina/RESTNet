...
public class PolicyProtectedTask extends Task<ITaskResult, String> {
	...
	if (config.isEnabled()) {
		...
		try {
			...
			policy.createGroupPolicy("member", ssoToken, getUri().toString(), new String[] { "GET", "PUT",
				"POST", "DELETE" });
		    }
		} catch (Exception x) {
		    x.printStackTrace();
		...
		try {
			...
			policy.createUserPolicy(results.get("uid"), ssoToken, newUri.toString(), new String[] { "GET",
				"PUT", "POST", "DELETE" });
		    else
