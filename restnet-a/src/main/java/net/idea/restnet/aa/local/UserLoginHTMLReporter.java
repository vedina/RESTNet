...
public class UserLoginHTMLReporter<U extends User> extends UserLoginURIReporter<U> {
	...
	try {
		...
		writer.write(String.format("<form method='post' action='%s/protected/signin?targetUri=%s'>",
			baseReference, redirect));

		writer.write(String.format(
			"<tr><th align='right'>%s</th><td><input type='text' size='40' name='%s' value=''></td></tr>",
			"User name:&nbsp;", "login"));
		writer.write(String
		...
		writer.write(String.format("<form method='post' action='%s/protected/signout?targetUri=%s'>",
			baseReference, redirect));
		writer.write(String.format("<tr><th align='right'>%s</th><td>%s</td></tr>", "User name:&nbsp;",
			item.getIdentifier()));

		for (Role role : getRequest().getClientInfo().getRoles()) {
		    writer.write(String.format("<tr><th align='right'>%s</th><td>%s</td></tr>", "Role:&nbsp;",
			    role.getDescription()));
		}
