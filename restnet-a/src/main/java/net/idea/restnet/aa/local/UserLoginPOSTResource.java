...
public class UserLoginPOSTResource<U extends User> extends CatalogResource<U> {
	...
	public IProcessor<Iterator<U>, Representation> createConvertor(
		...
		if (variant.getMediaType().equals(MediaType.TEXT_HTML)) {
			return new StringConvertor(createHTMLReporter(false),
					MediaType.TEXT_HTML);
		} else if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
			return new StringConvertor(new UserLoginURIReporter(getRequest(),
					getDocumentation()) {
				@Override
	...
	protected Reporter createHTMLReporter(boolean headles) {
		return new UserLoginHTMLReporter(getRequest(), getDocumentation(),
				getHTMLBeauty());
	}

	@Override
	protected Iterator<U> createQuery(Context context, Request request,
