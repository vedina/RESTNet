...
public class OpenSSOVerifier implements Verifier {
		...
		if ((token != null) && (!"".equals(token))) {
			...
			try {
				...
				if (ssoToken.isTokenValid()) {
					...
					request.getCookies().removeAll(OpenSSOCookie.CookieName);
					return enabled ? Verifier.RESULT_INVALID
							: Verifier.RESULT_VALID;
				}
			} catch (Exception x) {
				x.printStackTrace(); // TODO
				return enabled ? Verifier.RESULT_MISSING
						: Verifier.RESULT_VALID;
			}
		} else {
			request.getCookies().removeAll(OpenSSOCookie.CookieName);
	...
	protected void setUser(OpenSSOToken ssoToken, Request request)
			throws Exception {
	...
	protected User createUser(OpenSSOToken ssoToken, Request request)
		...
		request.getCookies().removeAll(OpenSSOCookie.CookieName);
		request.getCookies().add(OpenSSOCookie.bake(ssoToken.getToken(),useSecureCookie(request)));
		return user;
	}
