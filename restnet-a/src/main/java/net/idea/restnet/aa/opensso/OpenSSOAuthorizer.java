...
public class OpenSSOAuthorizer extends Authorizer {
	...
	Reference fullPrefix = new Reference(String.format("%s%s%s/", u, u.lastIndexOf("/") == u.length() - 1 ? ""
		: "/", prefix));

	u = ref.toString();
	Reference uri = new Reference(String.format("%s%s", u, u.lastIndexOf("/") == u.length() - 1 ? "" : "/"));
	u = ref.toString();
	Reference uri2check = new Reference(u == null ? null : u.lastIndexOf("/") == u.length() - 1 ? u
		: String.format("%s/", u)); // add trailing slash
	int prefix_len = fullPrefix.toString().length();
	while (!fullPrefix.equals(uri)) {
	    uri2check = uri;
	    uri = uri.getParentRef();
	    if (uri.toString().length() < prefix_len)
		return null; // smth wrong
	}
	u = uri.toString();
