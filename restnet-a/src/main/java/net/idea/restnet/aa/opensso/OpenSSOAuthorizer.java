...
public class OpenSSOAuthorizer extends Authorizer {
	...
	public String uri2check(Reference root, Reference ref) throws Exception {
		...
		Reference fullPrefix = new Reference(
				String.format("%s%s%s/", u, u.lastIndexOf("/") == u.length() - 1 ? "" : "/", prefix));

		u = ref.toString();
		Reference uri = new Reference(String.format("%s%s", u, u.lastIndexOf("/") == u.length() - 1 ? "" : "/"));
		u = ref.toString();
		Reference uri2check = new Reference(
				u == null ? null : u.lastIndexOf("/") == u.length() - 1 ? u : String.format("%s/", u)); // add
																										// trailing
																										// slash
