...
public class OT {
	public enum OTClass {
		...
		FeatureValue, Algorithm, Model, Parameter, Validation, ValidationInfo, Task, ErrorReport, OTMaterial;
		public String getNS() {
	...
	public enum OTProperty {
		...
		public Property createProperty(OntModel jenaModel) {
			Property p = jenaModel.getObjectProperty(String.format(_NS,
					toString()));
			return p != null ? p : jenaModel.createObjectProperty(String
					.format(_NS, toString()));
		}
	...
	public enum DataProperty {
		...
		public Property createProperty(OntModel jenaModel) {
			Property p = jenaModel.getDatatypeProperty(String.format(_NS,
					toString()));
			return p != null ? p : jenaModel.createDatatypeProperty(String
					.format(_NS, toString()));
		}
	...
	public static OntModel createModel(OntModelSpec spec)
	...
	public static Model createModel(Model base, InputStream in,
			MediaType mediaType) throws ResourceException {

		if (MediaType.APPLICATION_RDF_XML.equals(mediaType)) {
			Model model = base == null ? createModel(OntModelSpec.OWL_DL_MEM)
					: base;
	...
	public static Model createModel(Model model, Representation entity,
			MediaType mediaType) throws ResourceException {
		try {
			return createModel(model, entity.getStream(),
					entity.getMediaType() == null ? entity.getMediaType()
							: entity.getMediaType());
		} catch (IOException x) {
			throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
					x.getMessage(), x);
	...
	public static String getJenaFormat(MediaType mediaType)
			throws ResourceException {
	...
	public static void write(OntModel jenaModel, OutputStream output,
			MediaType mediaType, boolean isXml_abbreviation) throws IOException {
		try {
			...
			if (mediaType.equals(MediaType.APPLICATION_RDF_XML)) {
				...
				fasterWriter
						.setProperty("showDoctypeDeclaration", Boolean.TRUE);
			} else if (mediaType.equals(MediaType.APPLICATION_RDF_TURTLE))
				...
				fasterWriter.setProperty("showXmlDeclaration", Boolean.TRUE);
				fasterWriter
						.setProperty("showDoctypeDeclaration", Boolean.TRUE); // essential
																				// to
																				// get
																				// XSD
																				// prefixed
			...
			Context.getCurrentLogger().warning(
					x.getMessage() == null ? x.toString() : x.getMessage());
		} finally {
