package net.idea.restnet.rdf.reporter;
...
public class TaskRDFReporter<USERID> extends CatalogRDFReporter<UUID> {
	...
	Individual task = getJenaModel().createIndividual(
		String.format("%s%s/%s", baseRef, SimpleTaskResource.resource, item.getUuid()),
		OT.OTClass.Task.getOntClass(getJenaModel()));

	task.addLiteral(DC.title, getJenaModel().createTypedLiteral(item.getName(), XSDDatatype.XSDstring));

	task.addLiteral(DC.date, getJenaModel().createTypedLiteral(item.getStarted(), XSDDatatype.XSDdateTime));
	...
	task.addLiteral(OT.DataProperty.percentageCompleted.createProperty(getJenaModel()), getJenaModel()
		.createTypedLiteral(item.getPercentCompleted(), XSDDatatype.XSDfloat));
	...
	if (item.getError() != null) {
	    ...
	    error.addLiteral(OT.DataProperty.errorDetails.createProperty(getJenaModel()), exception.getStatus()
		    .getDescription());
	...
	if (item.isDone())
	    try {
		task.addLiteral(OT.DataProperty.resultURI.createProperty(getJenaModel()), getJenaModel()
			.createTypedLiteral(item.getUri().toString(), XSDDatatype.XSDanyURI));
	    } catch (Exception x) {
