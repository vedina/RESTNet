...
/**
 * Iterates over objects of a specified OT.class TODO make use of
...
public abstract class RDFObjectIterator<Item> implements Iterator<Item> {
    ...
    public RDFObjectIterator(Representation representation, MediaType mediaType, String topObject)
	...
	return newEntry.isURIResource()
		&& (newEntry.getURI().equals("http://www.opentox.org/api/1.1#NumericFeature")
			|| newEntry.getURI().equals("http://www.opentox.org/api/1.1#NominalFeature") || newEntry
			.getURI().equals("http://www.opentox.org/api/1.1#StringFeature"));
    }
    ...
    /**
     * This is mainly used to create {@link Template} to match record uris and
     * determine if they originate from this service
     * 
     * @param baseReference
     */
	...
	if (rdfNode.isResource()) {
	    Statement st = ((Resource) rdfNode).getProperty(property);
	    if (st != null)
		return ((Resource) rdfNode).getProperty(property).getObject();
	    else
		return null;
	} else
	    return null;
	...
	if (posCompound > 0)
	    return String.format("%s%s", uri.substring(0, posDataset), uri.substring(posCompound));
	return uri;
    }
