 ...
 * 
 */
public abstract class CatalogResource<T> extends
	...
	customizeVariants(new MediaType[] { MediaType.TEXT_URI_LIST, MediaType.APPLICATION_JSON,
		...
		MediaType.TEXT_RDF_NTRIPLES, MediaType.APPLICATION_JAVA_OBJECT, MediaType.TEXT_HTML,
		MediaType.TEXT_CSV, MediaType.APPLICATION_WADL });
    ...
    @Override
    public IProcessor<Iterator<T>, Representation> createConvertor(Variant variant) throws AmbitException,
	    ResourceException {
    ...
    public IProcessor<Iterator<T>, Representation> createRDFConvertor(Variant variant, String filenamePrefix)
	    throws AmbitException, ResourceException {
	throw new ResourceException(Status.SERVER_ERROR_NOT_IMPLEMENTED);
    }

    public IProcessor<Iterator<T>, Representation> createJSONConvertor(Variant variant, String filenamePrefix)
	    throws AmbitException, ResourceException {
	throw new ResourceException(Status.SERVER_ERROR_NOT_IMPLEMENTED);
    }

    public IProcessor<Iterator<T>, Representation> createCSVConvertor(Variant variant, String filenamePrefix)
	    throws AmbitException, ResourceException {
	throw new ResourceException(Status.SERVER_ERROR_NOT_IMPLEMENTED);
    }

    public IProcessor<Iterator<T>, Representation> createDefaultConvertor(Variant variant, String filenamePrefix)
	    throws AmbitException, ResourceException {
	return new StringConvertor(createURIReporter(), MediaType.TEXT_URI_LIST, filenamePrefix);
    }

    protected Reporter createURIReporter() {
	return new CatalogURIReporter<T>(getRequest(), getDocumentation());
    }
	...
	return String.format("Apply %s %s %s", item == null ? "" : item.toString(), source == null ? "" : "to",
		source == null ? "" : source);
    }

    @Override
    protected Representation processAndGenerateTask(Method method, Representation entity, final Variant variant,
	    final boolean async) throws ResourceException {
	...
	while (query.hasNext())
		...
		ITask<ITaskResult, String> task = ((ITaskApplication) getApplication()).addTask(
			getTaskTitle(model, reference), callable, getRequest().getRootRef(), getToken());
		task.update();
