...
public class TaskStorage<USERID> implements ITaskStorage<USERID> {
	...
	this.name = name;
	this.logger = logger;
	int nthreads = (int) Math.ceil(Runtime.getRuntime().availableProcessors() * cpuutilisation
		* (1 + waittime / cputime));
	// nthreads = 8;
	nthreads = 64;
	 ...
	 * /java/TPS01-J.+Do+
	 * not+execute+interdependent+tasks+in+a+bounded+thread+pool
	 */
	...
	while (keys.hasNext()) {
	    ...
	    try {
		// task.update();
		if (task.isDone() && (task.isExpired(taskCleanupRate)))
		    tasks.remove(key);
	    } catch (Exception x) {
	...
	// SynchronousQueue<Runnable> taskQueue = new
	...
	// ArrayBlockingQueue<Runnable>(5,true);

	ExecutorService xs = new ThreadPoolExecutor(maxThreads, maxThreads, 0L, TimeUnit.MILLISECONDS, taskQueue, tf,
		rjh) {
    ...
    @Override
    public ITask<ITaskResult, USERID> addTask(String taskName, ICallableTask callable, Reference baseReference,
	...
	ITaskResult ref = new TaskResult(String.format("%s%s/%s", baseReference.toString(),
		SimpleTaskResource.resource, Reference.encode(task.getUuid().toString())));
	task.setUri(ref);
	...
	if (tasks.get(task.getUuid()) == null) {
	    ...
	    try {
		Future future = task.isInternal() ? completionService_internal.submit(xtask, null)
    ...
    /*
     * public Iterator<UUID> getTasks() { ArrayList<UUID> lists = new
     * ArrayList<UUID>(); Iterator<UUID> i = tasks.keySet().iterator(); while
     * (i.hasNext()) lists.add(i.next()); return lists.iterator(); }
     */
