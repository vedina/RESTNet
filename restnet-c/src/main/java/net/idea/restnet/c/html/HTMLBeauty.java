...
public class HTMLBeauty {
	...
	if (jsGoogleAnalytics == null)
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(getClass().getClassLoader()
			.getResourceAsStream("net/idea/restnet/config/googleanalytics.js")));
	...
	w.write("<body>");
	w.write(String
		.format("<link rel=\"stylesheet\" href=\"%s/style/tablesorter.css\" type=\"text/css\" media=\"screen\" title=\"Flora (Default)\">\n",
			baseReference));
	w.write("\n");
	...
	try {
		...
		searchQuery = form.getFirstValue(AbstractResource.search_param) == null ? "" : form
	...
	return String
		.format("<div class='search ui-widget'>\n"
			+ "<p title='%s'>%s</p>\n"
			...
			+ "</table>\n" + "</form> \n" + "</div>\n", hint, getSearchTitle(), baseReference,
			getSearchURI(), searchQuery == null ? "" : searchQuery, pageSize);
    }
