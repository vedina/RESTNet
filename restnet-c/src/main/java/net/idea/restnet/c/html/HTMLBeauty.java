...
public class HTMLBeauty {
	...
	public void writeHTMLHeader(Writer w, String title, Request request, String meta, ResourceDoc doc)
			throws IOException {
	...
	public void writeTopHeader(Writer w, String title, Request request, String meta, ResourceDoc doc)
			throws IOException {
		Reference baseReference = request == null ? null : request.getRootRef();

		// Determine if the request is made by Microsoft Internet Explorer 7, as
		// many elements on the page break on it.
		if (request != null) {
		...
		w.write(String.format("<link rel=\"primarytopic\" type=\"application/rdf+xml\" href=\"%s\"/>", ref));
		w.write(String.format("<link rel=\"primarytopic\" type=\"text/n3\" href=\"%s\"/>", ref));
		...
		w.write("<script type='text/javascript'>" + "(function() {"
		...
		// src=\"%s/jquery/jquery.tablesorter.min.js\"></script>\n",baseReference));
		...
		// w.write("<script>$(function() {$( \".tabs\" ).tabs({event:
		// \"mouseover\",cache: true, ajaxOptions: {error: function( xhr,
		// status, index, anchor ) {$( anchor.hash ).html(status
		// );}}});});</script>");
		...
		w.write("<body>");
		w.write(String.format(
				"<link rel=\"stylesheet\" href=\"%s/style/tablesorter.css\" type=\"text/css\" media=\"screen\" title=\"Flora (Default)\">\n",
				baseReference));
		w.write("\n");
		...
		if (logoHeader != null)
			...
			header.append(String.format("<div class='logo_header'>%s</div>", baseReference, getTitle()));
		if (logoRight != null)
		...
		// Apply style for the hovered buttons sans (!) the currently selected
		// one.
		// There are better ways to do it, but this should be okay for now.
		// However, this breaks MSIE 7. Moreover, this browser gets crazy even
		// if
		// the change is implemented purely with simple CSS a:hover, and for
		// this
		// reason, we simply disable the mousever effect for it.
		if (!isMsie7) {
			w.write(String.format("<script>\n" +
		...
		/*
		 * left = ""; middle = ""; right =
		 * String.format("<a href='%s/%s'>%s</a>", baseReference.toString(),
		 * getLoginLink(),
	...
	public void writeTopLinks(Writer w, String title, Request request, String meta, ResourceDoc doc,
			Reference baseReference) throws IOException {
	...
	public void writeSearchForm(Writer w, String title, Request request, String meta, Method method)
			throws IOException {
	...
	protected String searchMenu(Reference baseReference, Form form) {
		...
		try {
			if ((form != null) && (form.size() > 0)) {
				searchQuery = form.getFirstValue(AbstractResource.search_param) == null ? ""
						: form.getFirstValue(AbstractResource.search_param);
		...
		return String.format(
				"<div class='search ui-widget'>\n" + "<p title='%s'>%s</p>\n"
						...
						+ "</table>\n" + "</form> \n" + "</div>\n",
				hint, getSearchTitle(), baseReference, getSearchURI(), searchQuery == null ? "" : searchQuery,
				pageSize);
	}
	...
	public String printWidgetHeader(String header) {
		return String.format("<div class=\"ui-widget \" style=\"margin-top: 20px; padding: 0 .7em;\">\n"
	...
	public String printWidgetContent(String content, String style) {
		return String.format("%s\n%s\n%s", printWidgetContentHeader(style), printWidgetContentContent(content),
				printWidgetContentFooter());
	}

	public String printWidget(String header, String content, String style) {
		return String.format("%s\n%s\n%s", printWidgetHeader(header), printWidgetContent(content, style),
				printWidgetFooter());

	}

	public String printWidget(String header, String content) {
		return String.format("%s\n%s\n%s", printWidgetHeader(header), printWidgetContent(content, ""),
				printWidgetFooter());
	...
	protected String printMenuItem(String relativeURI, String title, String baseReference, String pagesize,
		...
		return String.format("\t\t\t\t<li><a class='%s' title='%s' href='%s%s%s%s'>%s</a></li>\n",
				...
				hint == null ? title : hint, baseReference, relativeURI, pagesize == null ? "" : "?pagesize=",
				pagesize == null ? "" : pagesize, title);
	}

	public String getPaging(int page, int start, int last, long pageSize) {

		// Having a constant number of pages display on top is convenient for
		// the users and provides more consistent
		// overall look. But this would require the function to define different
		// input parameters. In order to not
		// break it, implement a workaround, by calculating how many pages the
		// caller (likely) intended to be shown.
		...
		// Disable this for the time being as it seems to not fit well into the
		// overall look.
		...
		// Display links to pages. Pages are counted from zero! Hence why we
		// display "i+1".
