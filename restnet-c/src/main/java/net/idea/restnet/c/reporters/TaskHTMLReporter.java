...
public class TaskHTMLReporter<USERID> extends CatalogURIReporter<UUID> {
	...
	try {
		...
		htmlBeauty.writeHTMLHeader(output, htmlBeauty.getTitle(), getRequest(),
			"<meta http-equiv=\"refresh\" content=\"10\">", getDocumentation());// ,"<meta http-equiv=\"refresh\" content=\"10\">");
	    }
	    printNavigation();
	} catch (Exception x) {
	...
	for (TaskStatus status : TaskStatus.values())
	    output.write(String.format("<a href='%s%s?search=%s&%s=%s'>%s</a>&nbsp;", baseReference,
	...
	try {
		...
		output.write(String.format(
			...
			baseReference.toString(), SimpleTaskResource.resource, item.getUuid(),
			new Date(item.getStarted()), item.isDone() ? (item.getError() == null ? ""
				: "ui-state-highlight") : "", item.getName(),
			...
			item.getError() != null ? "" : item.getTimeCompleted() > 0 ? new Date(item.getTimeCompleted())
				: "", baseReference.toString(), item.isDone() ? (item.getError() == null ? "tick.png"
				: "cross.png") : "progress.gif", (item.isDone() && item.getError() == null) ? t : "",
			(item.isDone() && item.getError() == null) ? "Results available" : "",
			...
			item.getPolicyError() == null ? "" : "Policy error", item.getPolicyError() == null ? "" : item
				.getPolicyError().getMessage()));
