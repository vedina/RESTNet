 ...
 * @param <P>
 */
public abstract class AbstractResource<Q, T, P extends IProcessor<Q, Representation>> extends ServerResource implements
	IAuthToken, IFreeMarkerSupport {
	...
	return new TemplateRepresentation(templateName,
		(Configuration) ((IFreeMarkerApplication) getApplication()).getConfiguration(), map,
		MediaType.TEXT_HTML);
    }
    ...
    protected abstract Representation processAndGenerateTask(final Method method, Representation entity,
	    final Variant variant, final boolean async) throws ResourceException;

    /*
     * 
     * 
     * @Override protected void describeGet(MethodInfo info) {
     * info.setIdentifier("item");
     * info.setDocumentation("To retrieve details of a specific item");
     * 
     * Iterator<Variant> vars = getVariants().iterator(); while (vars.hasNext())
     * { Variant var = vars.next(); RepresentationInfo repInfo = new
     * RepresentationInfo(var.getMediaType()); //repInfo.setXmlElement("item");
     * repInfo
     * .setDocumentation(String.format("%s representation",var.getMediaType()));
     * info.getResponse().getRepresentations().add(repInfo); }
     * 
     * 
     * FaultInfo faultInfo = new
     * FaultInfo(Status.CLIENT_ERROR_NOT_FOUND,"Not found");
	...
	for (Cookie cookie : request.getCookies()) {
	    if ("subjectid_secure".equals(cookie.getName()))
		try {
