...
public class ExecutableTask<USERID> extends FutureTask<ITaskResult> {
	...
	try {
		...
		task.setError((ResourceException) err);
	    else
		task.setError(new ResourceException(err));
     ...
     * 
     * try { if (future!=null) { Reference ref = future.get(100,
     * TimeUnit.MILLISECONDS); future = null; completed =
     * System.currentTimeMillis(); status = TaskStatus.Completed; setUri(ref); }
     * } catch (TimeoutException x) { } catch (ExecutionException x) { Throwable
     * err = x.getCause()==null?x:x.getCause(); if (err instanceof
     * ResourceException) error = (ResourceException) err; else error = new
     * ResourceException(err); status = TaskStatus.Error; } catch
     * (InterruptedException x) { error = null; status = TaskStatus.Cancelled; }
     * catch (CancellationException x) { error = null; status =
