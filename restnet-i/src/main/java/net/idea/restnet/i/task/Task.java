...
public class Task<REFERENCE, USERID> implements Serializable, ITask<REFERENCE, USERID> {
	...
	PROPERTY_PERCENT {
		...
		try {
		    if (value != null)
			task.setPercentCompleted(Float.parseFloat(value.toString()));
		} catch (Exception x) {
		}
	    }
	};
	...
	try {
	    return String.format("%s [%s] Started %s Completed %s [%s] [%s] %s", name == null ? "?" : name, getUri(),
		    new Date(started), ((completed > 0) ? new Date(completed) : "-"), getStatus(), userid,
    ...
    /*
     * public void propertyChange(PropertyChangeEvent evt) { try { TaskProperty
     * p = TaskProperty.valueOf(evt.getPropertyName()); p.update(this,
     * evt.getNewValue()); } catch (Exception x) {
     * 
     * }
     * 
     * }
    ...
    /**
     * Does nothing so far, but should register the result URI to the policy
     * server
     */
	...
	return String.format(format, uri, toString(), getName() == null ? "" : getName(), getError() == null ? ""
		: getError(), getPolicyError() == null ? "" : getPolicyError(), getStatus() == null ? "" : getStatus(),
		getStarted(), getTimeCompleted(), getUri() == null ? "" : getUri(), getUserid() == null ? ""
			: getUserid());
    }
