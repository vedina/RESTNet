...
public class PolicyQuery extends AbstractQuery<IRESTPolicy<Integer>, String, EQCondition, Boolean> implements
	IQueryRetrieval<Boolean>, IDBConfig {
	...
	return String
		.format("select prefix,resource,sum(m%s) from %spolicy join %suser_roles using(role_name) where user_name=? and prefix=? and resource=? and m%s=1 group by prefix,resource\n",
			getMethod().getName().toLowerCase(),
			databaseName == null ? "" : String.format("`%s`.", databaseName), databaseName == null ? ""
				: String.format("`%s`.", databaseName), getMethod().getName().toLowerCase());
    }
