...
public class RESTPolicyResource<Q extends IQueryRetrieval<IRESTPolicy<Integer>>> extends
	...
	if (variant.getMediaType().equals(MediaType.APPLICATION_JAVASCRIPT)) {
	    ...
	    return new OutputWriterConvertor(new RESTPolicyJSONReporter(baseRef, jsonpcallback),
		    MediaType.APPLICATION_JAVASCRIPT);
	} else
	    // if (variant.getMediaType().equals(MediaType.APPLICATION_JSON)) {
	    return new OutputWriterConvertor(new RESTPolicyJSONReporter(baseRef), MediaType.APPLICATION_JSON);
    }
    ...
    @Override
    protected CallableProtectedTask<String> createCallable(Method method, Form form, IRESTPolicy<Integer> item)
	    throws ResourceException {
	Connection conn = null;
	try {
	    DBConnection dbc = new DBConnection(getApplication().getContext(), getConfigFile());
	    conn = dbc.getConnection();
	    return new CallablePolicyCreator(item, method, form, getRequest().getRootRef().toString(), conn,
		    getToken(), getDefaultUsersDB());
	} catch (Exception x) {
	    try {
		conn.close();
	    } catch (Exception xx) {
