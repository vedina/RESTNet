...
public class DbEnroller implements Enroler {
	...
	public synchronized void enrole(ClientInfo clientInfo) {
		if ((clientInfo.getUser() == null) || (clientInfo.getUser().getIdentifier() == null)
				|| ("".equals(clientInfo.getUser().getIdentifier())))
			return;
	...
	protected Role createRole(String name) {
		return new DBRole(name,
				String.format("%s%s", name.substring(0, 1).toUpperCase(), name.substring(1).replace("_", " ")));
	}
