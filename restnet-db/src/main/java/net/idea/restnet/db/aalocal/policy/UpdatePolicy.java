...
public class UpdatePolicy extends AbstractUpdate<DBRole, IRESTPolicy<Integer>> implements IDBConfig {
    ...
    public static String sql = "update %spolicy set role_name=?,prefix=?,resource=?,level=?,mget=?,mput=?,mpost=?,mdelete=? where idpolicy=?";

    public UpdatePolicy() {
	this(null);
    }

    public UpdatePolicy(IRESTPolicy<Integer> policy) {
	setObject(policy);
    }
	...
	if ((getObject() == null) || (getObject().getRole() == null) || (getObject().getId() == null)
		|| (getObject().getId() <= 0) || (getObject().getUri() == null))
	...
	try {
	    ...
	    params2.add(new QueryParam<Integer>(Integer.class, getObject().getId()));
	    return params2;
	} catch (Exception x) {
