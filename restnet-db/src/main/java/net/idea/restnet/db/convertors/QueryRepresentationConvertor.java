...
/**
 * An abstract {@link IProcessor} , converting between a query results
 * {@link IQueryRetrieval} and restlet Representation.
 * 
 * @author nina
 * 
 * @param <T>
...
public abstract class QueryRepresentationConvertor<T, Q extends IQueryRetrieval<T>, Output>
		extends
		RepresentationConvertor<T, Q, Output, QueryAbstractReporter<T, Q, Output>> {
	...
	public QueryRepresentationConvertor(
			QueryAbstractReporter<T, Q, Output> reporter) {
		this(reporter, MediaType.TEXT_PLAIN, null);
	}

	public QueryRepresentationConvertor(
			QueryAbstractReporter<T, Q, Output> reporter, MediaType media) {
	...
	public QueryRepresentationConvertor(
		...
		if (this.reporter != null)
			((QueryAbstractReporter<T, Q, Output>) this.reporter)
					.setMaxRecords(5000);
	}
