...
/**
 * 
 * @author nina Creates entire object and then converts it to a representation,
 *         no streaming
 * @param <T>
 * @param <Q>
 */
public abstract class AbstractObjectConvertor<T, Q extends IQueryRetrieval<T>, Output> extends
	QueryRepresentationConvertor<T, Q, Output> {

    public AbstractObjectConvertor(QueryReporter<T, Q, Output> reporter) {
	this(reporter, MediaType.TEXT_PLAIN);
	if (this.reporter != null)
	    ((QueryReporter<T, Q, Output>) this.reporter).setMaxRecords(5000);
    }
