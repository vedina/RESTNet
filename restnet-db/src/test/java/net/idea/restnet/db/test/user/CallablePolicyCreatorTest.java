...
public class CallablePolicyCreatorTest extends DbUnitTest {
	...
	CallablePolicyCreator callable = new CallablePolicyCreator(null, Method.POST, form,
		"http://localhost:8080/ambit2", c.getConnection(), null, getDatabase());
	TaskResult task = callable.call();

	c.close();
	c = getConnection();
	ITable table = c.createQueryTable("EXPECTED",
		String.format("SELECT role_name,mget,mput,mpost,mdelete from policy where resource='/model/123'  "));
	...
	policy.setId(1);
	CallablePolicyCreator callable = new CallablePolicyCreator(policy, Method.PUT, form,
		"http://localhost:8080/ambit2", c.getConnection(), null, getDatabase());
	TaskResult task = callable.call();
	c.close();
	c = getConnection();
	...
	RESTPolicy policy = new RESTPolicy();
	policy.setId(1);
	CallablePolicyCreator callable = new CallablePolicyCreator(policy, Method.DELETE, form,
		"http://localhost:8080/ambit2", c.getConnection(), null, getDatabase());
	TaskResult task = callable.call();
	c.close();
	c = getConnection();
	ITable table = c.createQueryTable("EXPECTED",
		String.format("SELECT role_name,mget,mput,mpost,mdelete from policy where idpolicy=1  "));
