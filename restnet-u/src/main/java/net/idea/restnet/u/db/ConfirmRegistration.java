...
public class ConfirmRegistration extends AbstractUpdate<IUser,UserRegistration>  implements IDBConfig {
	private String sql = "update %s.user_registration set confirmed=now(),status='confirmed' where code=? and status='commenced' and date_add(created,interval 2 day)>=now()";
	public ConfirmRegistration(IUser user,UserRegistration reg) {
		super();
		setObject(reg);
		setGroup(user);
	}
